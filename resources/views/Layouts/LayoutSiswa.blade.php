...
<head>
    ...
    <title>@yield('title') - Siswa SMKN 17</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .sidebar {
            background: linear-gradient(180deg, #3c1e70 0%, #6f42c1 100%);
            color: #fff;
            display: flex;
        ...
        .navbar-custom .user-avatar {
            width: 36px; height: 36px; background: linear-gradient(135deg, var(--primary), #a855f7);
...
<div class="sidebar offcanvas-lg offcanvas-start" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
    ...
    <div class="sidebar-menu">
        ...
        @php
            $unreadMailboxCount = \Illuminate\Support\Facades\Auth::check()
                ? \App\Models\StudentMailbox::where('student_id', \Illuminate\Support\Facades\Auth::id())->where('status', 'unread')->count()
                : 0;
        @endphp

        <a href="{{ route('siswa.mailbox.index') }}"
           class="{{ request()->routeIs('siswa.mailbox.*') ? 'active' : '' }} d-flex justify-content-between align-items-center">
            <div>
                <i class="bi bi-envelope-paper me-2"></i> Kotak Surat
            </div>
            @if ($unreadMailboxCount > 0)
                <span class="badge bg-danger rounded-pill px-2 py-1">{{ $unreadMailboxCount }}</span>
            @endif
        </a>
    </div>
