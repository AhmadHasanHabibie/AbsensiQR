...
<head>
    ...
    <title>Verifikasi PIN - Super Administrator</title>
...
<body>
    ...
    <div class="pin-card p-4 p-md-5">
        <div class="text-center mb-4">
            ...
            <span class="badge bg-{{ $isLocked ? 'danger' : 'info' }}-subtle text-{{ $isLocked ? 'danger' : 'info' }} border border-{{ $isLocked ? 'danger' : 'info' }}-subtle px-3 py-1 font-monospace fw-bold mb-2">
                {{ $isLocked ? 'ACCOUNT COOLDOWN LOCKED' : '2-STAGE AUTHENTICATION' }}
            </span>
            <h3 class="fw-bold text-white mb-2">Verifikasi PIN</h3>
            <p class="text-secondary small mb-0">
                {{ $isLocked ? 'Akun Super Administrator dikunci sementara demi keamanan sistem.' : 'Masukkan PIN keamanan 6-digit untuk melanjutkan ke Dashboard Super Administrator.' }}
            </p>
        </div>
        ...
        <!-- PIN INPUT FORM -->
        <form action="{{ route('superadmin.pin.process') }}" method="POST" autocomplete="off" id="pinForm">
            @csrf
            ...
            <div class="d-grid gap-2 mb-3">
                <button type="submit" id="submitBtn" class="btn btn-info font-monospace fw-bold py-3 rounded-3 shadow" {{ $isLocked ? 'disabled' : '' }}>
        ...
        <div class="text-center border-top border-secondary border-opacity-25 pt-3 mt-4">
            <form action="{{ route('logout') }}" method="POST" class="d-inline">
                ...
                <button type="submit" class="btn btn-link text-secondary text-decoration-none small">
                    <i class="bi bi-box-arrow-left me-1"></i> Batal & Logout System Owner
                </button>
            </form>
        </div>
    </div>
