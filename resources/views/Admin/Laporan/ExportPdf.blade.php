...
<head>
    ...
    <style>
        ...
        .class-info { width: 100%; border-collapse: collapse; margin: 0 0 8px; }
        .class-info td { border: 1px solid #bbb; padding: 5px 8px; }
        .data { width: 100%; border-collapse: collapse; }
        .data th { background: #e9ecef; border: 1px solid #777; padding: 6px 5px; text-align: center; }
        .data td { border: 1px solid #999; padding: 5px; }
        .text-left { text-align: left; }
        .text-center { text-align: center; }
        .text-right { text-align: right; }
        .class-summary { width: 48%; margin: 8px 0 0 auto; border-collapse: collapse; }
...
<body>
    <div class="school-header">
        <h2>SISTEM ABSENSI QR CODE</h2>
        <h3>SMKN 17 JAKARTA</h3>
        <p>Laporan Rekap Absensi Siswa</p>
    </div>

    <table class="report-info">
        <tr><td width="14%"><strong>Periode</strong></td><td>: {{ $dateLabel ?? '-' }}</td></tr>
        <tr><td><strong>Dicetak Oleh</strong></td><td>: Admin</td></tr>
        <tr><td><strong>Total Terlambat</strong></td><td>: {{ $terlambat ?? 0 }}</td></tr>
    </table>
    ...
    @forelse ($classGroups as $group)
        <div class="class-section">
            ...
            <table class="class-info">
                <tr>
                    <td width="50%"><strong>Wali Kelas:</strong> {{ $group['teacher_name'] }}</td>
                    <td><strong>Jumlah Siswa:</strong> {{ $group['students']->count() }}</td>
                </tr>
            </table>
            ...
            <table class="data">
                <thead>
                    <tr>
                        <th width="4%">No</th>
                        <th width="27%" class="text-left">Nama Siswa</th>
                        <th width="13%" class="text-left">NIS</th>
                        <th width="8%">Hadir</th>
                        <th width="10%">Terlambat</th>
                        <th width="8%">Izin</th>
                        <th width="8%">Sakit</th>
                        <th width="8%">Alpa</th>
                        <th width="14%">Persentase Kehadiran</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($group['students'] as $student)
                        <tr>
                            ...
                            <td class="text-right">{{ $student['hadir'] }}</td>
                            <td class="text-right">{{ $student['terlambat'] }}</td>
                            <td class="text-right">{{ $student['izin'] }}</td>
                            <td class="text-right">{{ $student['sakit'] }}</td>
                            <td class="text-right">{{ $student['alpa'] }}</td>
