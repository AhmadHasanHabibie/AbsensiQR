...
class DuplicateAccountService
{
     ...
     * NIP diperbolehkan sama untuk akun dengan credential/role yang berbeda (Multi-Account NIP).
     *
     * @param  string   $nip         NIP yang akan dicek
     * @param  int|null $excludeId   ID user yang dikecualikan (saat edit)
     * @return string|null           Selalu mengembalikan null (NIP diperbolehkan duplikat)
     */
     ...
     *
     * Mendeteksi apakah username sudah pernah muncul di baris sebelumnya dalam file yang sama.
     *
     * @param  string $username
     * @param  string $nip
     * @param  array  &$processedInFile  Array berisi ['username' => true] baris sebelumnya
    ...
    public function checkDuplicateInFile(string $username, string $nip, array &$processedInFile): ?string
    {
        ...
        if (isset($processedInFile[$key])) {
            return 'Data duplikat ditemukan di dalam file Excel (username sudah ada pada baris sebelumnya).';
        }
