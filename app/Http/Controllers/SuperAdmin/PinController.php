...
{
    /**
     * Menampilkan form verifikasi PIN 6-digit Super Administrator.
     */
    public function showVerifyForm()
    {
        ...
        return view('SuperAdmin.Auth.VerifyPin', [
    ...
    /**
     * Proses verifikasi PIN 6-digit Super Administrator.
     */
    public function processVerify(Request $request)
    {
        ...
        if ($cooldownData['active']) {
            return back()
                ->withErrors([
                    'pin' => "Akun Super Administrator dikunci sementara. Silakan coba kembali dalam " . sprintf('%02d:%02d', floor($cooldownData['seconds'] / 60), $cooldownData['seconds'] % 60) . "."
                ])
        ...
        $attemptsKey = "pin_attempts_user_{$user->id}";
        $cooldownKey = "pin_cooldown_user_{$user->id}";
        ...
        if ($user->verifyPin($request->pin)) {
            ...
            Cache::forget("pin_cooldown_logged_user_{$user->id}");
            ...
            ActivityLog::log(
                'PIN Benar',
                'Authentication 2FA',
                "PIN 6-digit berhasil diverifikasi untuk Super Administrator {$user->name}.",
                $user
            );
        ...
        if ($attempts < 3) {
            ...
            ActivityLog::log(
                ...
                'Security 2FA',
                "Percobaan PIN ke-{$attempts} gagal untuk Super Administrator {$user->name}. Sisa percobaan: {$remaining}.",
                $user
            );
        ...
        Cache::put("pin_cooldown_logged_user_{$user->id}", true, $cooldownDuration);

        ActivityLog::log(
            'Cooldown Dimulai',
            'Security 2FA',
            "Batas 3 kali kesalahan PIN tercapai. Cooldown 3 menit diaktifkan untuk Super Administrator {$user->name}.",
            $user
        );

        return back()
            ->withErrors([
                'pin' => 'Akun Super Administrator dikunci sementara selama 3 menit karena 3 kali kesalahan PIN.'
            ])
    ...
    {
        $cooldownKey = "pin_cooldown_user_{$userId}";
        $attemptsKey = "pin_attempts_user_{$userId}";
        $loggedKey   = "pin_cooldown_logged_user_{$userId}";
        ...
        if (Cache::has($loggedKey)) {
            ...
            ActivityLog::log(
                'Cooldown Berakhir',
                'Security 2FA',
                "Masa cooldown 3 menit telah berakhir untuk Super Administrator {$user->name}. Counter percobaan di-reset.",
                $user
            );
